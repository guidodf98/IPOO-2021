...
include "TeatroAct.php";
include "CineAct.php";
include "MusicalesAct.php";
    ...
    switch ($opcion) {
      ...
      case 7:
        if ($teatro->getFunciones() == null) {
          ...
          print ("\nEl costo de alquilar el teatro es de : $" . $teatro->darCosto());
        }
        break;
      default:
        "\nOpcion incorrecta\n";
...
function menu() {
  ...
  echo "\n| 7) Calcular el costo de alquiler del teatro |";
  echo "\n| 0) Salir                                    |";
  echo "\n ---------------------------------------------";
  echo "\nIngrese una opción: ";
...
function pedirDatosFunciones($horariosEstablecidos) {
  ...
  for ($i = 0; $i < $cant; $i++) {
    echo "\n-- Funcion " . ($i + 1) . " --\n";
    echo "Tipo de funcion: ";
    $exito = false;
    while (!$exito) {
      print "\nElija el tipo de activadad (teatro, cine, musical): ";
      $tipo = trim(fgets(STDIN));
      if ($tipo == "teatro" || $tipo == "cine" || $tipo == "musical") {
        $exito = true;
      } else {
    ...
    switch ($tipo) {
      case 'teatro':
        $funciones[$i] = new TeatroAct($nombre, $horaInicio, $duracion, $precio);
        break;
      case 'cine':
        echo "\nIngrese el genero: ";
        ...
        $funciones[$i] = new CineAct($nombre, $horaInicio, $duracion, $precio, $genero, $paisOrigen);
        break;
      case 'musical':
        echo "\nIngrese el director: ";
        $director = trim(fgets(STDIN));
        echo "\nIngrese la cantidad de personas en escenario: ";
        $cantPersonas = trim(fgets(STDIN));
        $funciones[$i] = new MusicalesAct($nombre, $horaInicio, $duracion, $precio, $director, $cantPersonas);
        break;
      default:
        print "Tipo de actividad incorrecta";
        break;
    }
  }
  return $funciones;
